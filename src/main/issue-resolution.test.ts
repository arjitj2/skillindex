...
describe('resolveInventoryIssue', () => {
  ...
  it('resolves plugin-owned MCPs and verifies the written definitions can connect', async () => {
    ...
    expect(checkedTargets.filter((target) => target.status !== 'verified')).toEqual([]);
    expect(verifiedAgentIds).toEqual(resolvedAgentIds);
    expectVerifiedSignalMapTarget(checkedTargets, 'sandbox-codex');
    expectVerifiedSignalMapTarget(checkedTargets, 'sandbox-factory');
  }, 20_000);
