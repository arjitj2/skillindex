...
async function collectMcpRecords(
  ...
  for (const owner of owners) {
    ...
    for (const entry of result.entries) {
      const inventoryMcpName = createInventoryMcpName(entry.name, result.owner);
...
function createInventoryMcpName(mcpName: string, owner: McpOwnerRecord): string {
  if (owner.plugin) {
    return `${owner.plugin.pluginName}:${mcpName}`;
  }
