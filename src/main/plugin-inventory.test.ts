...
describe('plugin inventory', () => {
  ...
  it('treats plugin MCPs as missing from agents without requiring normal MCPs in plugin configs', async () => {
    ...
    expect(inventory.mcps?.find((mcp) => mcp.name === 'healthyMcp')).toMatchObject({
      status: 'healthy',
      issueReasons: [],
      missingLocations: [],
    });
